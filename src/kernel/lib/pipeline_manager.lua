...
local function _rotateLogDir(sDir, nMax)
    ...
    for _, sName in ipairs(tList) do
        local sClean = sName
        if sClean:sub(-1) == "/" then
            sClean = sClean:sub(1, -2)
        end
...
local function _setupLogRotation(tConfig)
    ...
    pcall(function()
        syscall("raw_component_invoke", vfs_state.oRootFs.address, "makeDirectory", sLogDir)
    end)
    pcall(function()
        syscall("raw_component_invoke", vfs_state.oRootFs.address, "makeDirectory", sVblDir)
    end)
    ...
    if bOk1 and hLog then
        local bT1, bT2, sT3 = syscall("raw_component_invoke", vfs_state.oRootFs.address, "write", hLog,
            "=== LOG SESSION START ===\n")
        syscall("kernel_log", "[PM] .log test write: " .. tostring(bT1) .. " " .. tostring(bT2) .. " " .. tostring(sT3))
    end
    if bOk2 and hVbl then
        local bT1, bT2, sT3 = syscall("raw_component_invoke", vfs_state.oRootFs.address, "write", hVbl,
            "=== VBL SESSION START ===\n")
        syscall("kernel_log", "[PM] .vbl test write: " .. tostring(bT1) .. " " .. tostring(bT2) .. " " .. tostring(sT3))
    end
...
local function _writeToLogFiles(sText)
    if not g_tLogState or not sText or #sText == 0 then
        return
    end

    -- .vbl gets EVERYTHING
    if g_tLogState.hVblFile then
        local bOk, bWriteOk, sErr = syscall("raw_component_invoke", vfs_state.oRootFs.address, "write",
            g_tLogState.hVblFile, sText)
        if not bOk or not bWriteOk then
            -- Can't use kernel_log here (would recurse), so write directly
            -- to ringfs if available
            if g_nRingFsId then
                pcall(function()
                    _doInternalWrite(g_nRingFsId, "[PM] VBL WRITE FAILED: " .. tostring(sErr) .. "\n")
                end)
            end
        end
    end
    ...
    if g_tLogState.hLogFile then
        ...
        for sLine in (sText .. "\n"):gmatch("([^\n]*)\n") do
            if #sLine > 0 then
                local bSkip = false
                if sLine:find("%[DEBUG %]") then
                    bSkip = true
                end
                if sLine:find("%[ DEV  %]") then
                    bSkip = true
                end
                if sLine:find("%[SCHED %]") then
                    bSkip = true
                end
                if sLine:find("%[ IPC  %]") then
                    bSkip = true
                end
                if sLine:find("%[DK%]") then
                    bSkip = true
                end
                if not bSkip then
        ...
        if #sFiltered > 0 then
            local bOk, bWriteOk, sErr = syscall("raw_component_invoke", vfs_state.oRootFs.address, "write",
                g_tLogState.hLogFile, sFiltered)
            if not bOk or not bWriteOk then
                if g_nRingFsId then
                    pcall(function()
                        _doInternalWrite(g_nRingFsId, "[PM] LOG WRITE FAILED: " .. tostring(sErr) .. "\n")
                    end)
                end
            end
        end
...
local function _drainKernelLog()
    ...
    if not sNewLog or #sNewLog == 0 then
        return
    end
    ...
    if g_nRingFsId then
        pcall(function()
            _doInternalWrite(g_nRingFsId, sNewLog .. "\n")
        end)
    end
...
local function save_perms()
    ...
    for sPath, tInfo in pairs(tPermCache) do
        sData = sData ..
                    string.format('  ["%s"] = { uid = %d, gid = %d, mode = %d },\n', sPath, tInfo.uid or 0,
                tInfo.gid or 0, tInfo.mode or 755)
    end
    sData = sData .. "}"
...
local function check_access(nPid, sPath, sMode)
    ...
    if nReq == 4 then
        if nPermDigit >= 4 then
            bAllowed = true
        end
    elseif nReq == 2 then
        if nPermDigit == 2 or nPermDigit == 3 or nPermDigit == 6 or nPermDigit == 7 then
            bAllowed = true
        end
    end
...
function _doInternalOpen(sPath, sMode)
    ...
    local bOk, hRaw, sReason = syscall("raw_component_invoke", vfs_state.oRootFs.address, "open", sPath, sMode)
    if not hRaw then
        return nil, sReason
    end
    g_tPmInternal[nId] = {
...
function _doInternalRead(nId, nCount)
    ...
    if t.type == "file" then
        local r1, r2 = syscall("raw_component_invoke", vfs_state.oRootFs.address, "read", t.rawHandle, nCount)
...
function vfs_state.handle_open(nSenderPid, sSynapseToken, sPath, sMode)
    ...
    if sPath:sub(1, 5) == "/dev/" then
        ...
        local bOk, hRaw = syscall("raw_component_invoke", vfs_state.oRootFs.address, "open", sPath, sMode)
        if not hRaw then
    ...
    if not pObj then
        return nil, "ObCreateObject failed"
    end

    -- choose access mask from mode
    local nAccess = OB_ACCESS_READ
    if sMode == "w" or sMode == "a" then
        nAccess = OB_ACCESS_WRITE
    end
    if sMode == "rw" then
        nAccess = OB_ACCESS_READ + OB_ACCESS_WRITE
    end
...
function vfs_state.handle_read(nSenderPid, sSynapseToken, vHandle, nCount)
    ...
    if b.sCategory == "device" then
        ...
        local r1, r2 = syscall("raw_component_invoke", vfs_state.oRootFs.address, "read", b.hRawHandle, nCount)
        if type(r2) == "boolean" then
            r2 = nil
        end
        return r1, r2
...
function vfs_state.handle_close(nSenderPid, sSynapseToken, vHandle)
    ...
    elseif b.hRawHandle then
        syscall("raw_component_invoke", vfs_state.oRootFs.address, "close", b.hRawHandle)
    end
...
function vfs_state.handle_chmod(nSenderPid, sPath, nMode)
    ...
    if nUid ~= 0 and tEntry.uid ~= nUid then
        syscall("kernel_log",
            "[PM] CHMOD DENIED: PID " .. nSenderPid .. " (UID " .. nUid .. ") tried to touch " .. sPath)
...
function vfs_state.handle_delete(nSenderPid, sPath)
    ...
    if sPath:sub(1, 5) == "/dev/" then
        return nil, "Cannot delete device nodes"
    end
    if sPath:sub(1, 5) == "/boot" then
        return nil, "Cannot delete boot files"
    end
    ...
    local bOk, sResult = syscall("raw_component_invoke", vfs_state.oRootFs.address, "remove", sPath)
    if bOk then
        return true
    else
        return nil, tostring(sResult)
    end
end

function vfs_state.handle_mkdir(nSenderPid, sPath)
    if sPath:sub(1, 5) == "/dev/" then
        return nil, "Cannot mkdir in /dev"
    end
    ...
    local bOk, sResult = syscall("raw_component_invoke", vfs_state.oRootFs.address, "makeDirectory", sPath)
    if bOk then
        return true
    else
        return nil, tostring(sResult)
    ...
    while true do
        ...
        if bOk and nSender == nDkmsPid then
            if sSig == "load_driver_result" and p1 == nSenderPid then
                ...
                if not sDrvName then
                    sDrvName = "Unknown"
                end
                if nStatus == 0 then
                    local sMsg = (nDrvPid == 0) and string.format("[PM] Success: %s", sDrvName) or
                                     string.format("[PM] Success: Loaded '%s' (PID %d)", sDrvName, nDrvPid)
...
local function wait_with_throbber(sMessage, nSeconds)
    ...
    while computer.uptime() < nDeadline do
        if oGpu then
            ...
            if nPos >= nThrobberWidth then
                nPos = (nThrobberWidth * 2 - 2) - nPos
            end
            local sLine = "("
    ...
    while true do
        ...
        if bOk then
            if nSender == nDkmsPid and sSig == "load_driver_result" and p1 == nMyPid then
                -- p2=status, p3=driverName, p4=driverPid
                return p2, p3, p4
            elseif sSig == "os_event" then
                syscall("signal_send", nDkmsPid, "os_event", p1, p2, p3, p4, p5)
            elseif nSender == nDkmsPid and sSig == "syscall_return" then
                -- DKMS internal result, not ours — buffer it
                table.insert(g_tPmSignalBuffer, {nSender, sSig, p1, p2, p3, p4, p5})
            else
                table.insert(g_tPmSignalBuffer, {nSender, sSig, p1, p2, p3, p4, p5})
            end
...
local function _topoSortDrivers(tEnabled, tByName)
    ...
    -- Dependencies are pulled ahead of their dependents,
    -- then stable-sorted by priority within each "depth level".
    local tResult = {}
    local tVisited = {}
    local tInStack = {}

    local function visit(sName)
        if tVisited[sName] then
            return true
        end
        if tInStack[sName] then
            return false, "Circular dependency involving '" .. sName .. "'"
        end

        local tEntry = tByName[sName]
        if not tEntry then
            return true
        end -- missing dep: skip silently

        tInStack[sName] = true

        for _, sDep in ipairs(tEntry.depends or {}) do
            if not tByName[sDep] then
                syscall("kernel_log", "[PM] WARN: Driver '" .. sName .. "' depends on '" .. sDep ..
                    "' which is not configured or disabled")
            end
    ...
    -- Sort by priority first so topo-sort visits lower-priority nodes first
    local tPriSorted = {}
    for _, e in ipairs(tEnabled) do
        tPriSorted[#tPriSorted + 1] = e
    end
    table.sort(tPriSorted, function(a, b)
        return (a.priority or 500) < (b.priority or 500)
    end)

    for _, tEntry in ipairs(tPriSorted) do
        local bOk, sErr = visit(tEntry.name)
        if not bOk then
            syscall("kernel_log", "[PM] ERROR: " .. sErr)
            return nil, sErr
        end
    end
...
local function _checkDriverFileExists(sPath)
    local bOk, hTest = syscall("raw_component_invoke", vfs_state.oRootFs.address, "open", sPath, "r")
...
local function process_autoload()
    ...
    if bLcOk and hLoaderCfg then
        ...
        syscall("raw_component_invoke",
            vfs_state.oRootFs.address, "close", hLoaderCfg)
        if sLcData and #sLcData > 0 then
            local fLc = load(sLcData, "loader.cfg", "t", {})
            if fLc then
                local bPcall, tLoaderCfg = pcall(fLc)
                if bPcall and type(tLoaderCfg) == "table" then
                    sDriversCfgPath = tLoaderCfg.drivers_cfg
                    if sDriversCfgPath and #sDriversCfgPath > 0 then
                        syscall("kernel_log", "[PM] loader.cfg → drivers_cfg = " .. sDriversCfgPath)
                    else
                        sDriversCfgPath = nil
                    end
                end
    ...
    -- ── Fallback chain ──
    local tCfgPaths = {}
    if sDriversCfgPath then
        tCfgPaths[#tCfgPaths + 1] = sDriversCfgPath
    end
    tCfgPaths[#tCfgPaths + 1] = "/boot/sys/drivers.cfg"
    tCfgPaths[#tCfgPaths + 1] = "/etc/drivers.cfg"

    local tDrivers = nil
    local bNewFormat = false
    local sUsedPath = nil
    ...
    for _, sCfgPath in ipairs(tCfgPaths) do
        ...
        if bOk and hFile then
            ...
            syscall("raw_component_invoke",
                vfs_state.oRootFs.address, "close", hFile)
            if sData and #sData > 0 then
                local f, sParseErr = load(sData, "drivers.cfg", "t", {})
                if f then
                    local bPcall, tResult = pcall(f)
                    if bPcall and type(tResult) == "table" then
                        tDrivers = tResult
                        bNewFormat = true
                        sUsedPath = sCfgPath
                    ...
                    syscall("kernel_log", "[PM] ERROR: " .. sCfgPath ..
                        " parse error: " .. tostring(sParseErr))
                end
            end
        end
    end

    -- ── Fallback to old /etc/autoload.lua ──
    ...
    if not bNewFormat then
        ...
        local bAOk, hAuto = syscall("raw_component_invoke", vfs_state.oRootFs.address, "open", "/etc/autoload.lua", "r")
        if bAOk and hAuto then
            local _, sAData = syscall("raw_component_invoke", vfs_state.oRootFs.address, "read", hAuto, math.huge)
                ...
                if fA then
                    ...
                    if tList then
                        for _, sDrvPath in ipairs(tList) do
                            syscall("kernel_log", "[PM] Autoloading (legacy): " .. sDrvPath)
    ...
    for _, tEntry in ipairs(tDrivers) do
        if not tEntry.name or not tEntry.path then
            syscall("kernel_log", "[PM] WARN: Skipping malformed entry (missing name or path)")
    ...
    if #tEnabled == 0 then
        syscall("kernel_log", "[PM] No drivers to load.")
        return
    end

    -- ── Dependency check: warn about missing deps ──
    for _, tEntry in ipairs(tEnabled) do
        for _, sDep in ipairs(tEntry.depends or {}) do
            if not tByName[sDep] then
                -- Check if it's disabled rather than absent
                local bFound = false
                for _, tAll in ipairs(tDrivers) do
                    if tAll.name == sDep then
                        bFound = true;
                        break
                    end
                end
                if bFound then
                    syscall("kernel_log",
                        "[PM] WARN: '" .. tEntry.name .. "' depends on '" .. sDep .. "' which is DISABLED")
                else
                    syscall("kernel_log",
                        "[PM] WARN: '" .. tEntry.name .. "' depends on '" .. sDep .. "' which is NOT CONFIGURED")
                end
            end
        end
    end

    -- ── Topological sort (respects dependencies + priority) ──
    local tSorted, sSortErr = _topoSortDrivers(tEnabled, tByName)
    if not tSorted then
        syscall("kernel_log", "[PM] FATAL: Dependency resolution failed: " .. tostring(sSortErr))
        syscall("kernel_log", "[PM] Falling back to priority-only order...")
        tSorted = tEnabled
    ...
    -- ── Load drivers in resolved order ──
    local tLoaded = {} -- [name] = true if successfully loaded
    local nOk, nFail = 0, 0

    for nIdx, tEntry in ipairs(tSorted) do
        -- Check dependencies actually loaded
        local bDepsMet = true
        for _, sDep in ipairs(tEntry.depends or {}) do
            if tByName[sDep] and not tLoaded[sDep] then
                bDepsMet = false
                syscall("kernel_log",
                    "[PM] SKIP: '" .. tEntry.name .. "' — dependency '" .. sDep .. "' failed to load")
                break
            end
        end

        if not bDepsMet then
            nFail = nFail + 1
            goto next_driver
        end

        -- Check file exists before bothering DKMS
        if not _checkDriverFileExists(tEntry.path) then
            syscall("kernel_log", "[PM] FAIL: '" .. tEntry.name .. "' — file not found: " .. tEntry.path)
            nFail = nFail + 1
            goto next_driver
        end

        -- Load via DKMS (with result tracking)
        syscall("kernel_log", string.format("[PM] [%d/%d] Loading '%s' (pri=%d) %s", nIdx, #tSorted, tEntry.name,
            tEntry.priority or 500, tEntry.path))

        local nStatus, sDrvName, nDrvPid = _loadDriverWithResult(tEntry.path)

        if nStatus == 0 then
            tLoaded[tEntry.name] = true
            nOk = nOk + 1
            local sMsg = sDrvName or tEntry.name
            if nDrvPid and nDrvPid > 0 then
                sMsg = sMsg .. " (PID " .. nDrvPid .. ")"
            end
            syscall("kernel_log", "[PM] OK: " .. sMsg)
        else
            nFail = nFail + 1
            syscall("kernel_log", "[PM] FAIL: '" .. tEntry.name .. "' — status " .. tostring(nStatus))
        end
    ...
    syscall("kernel_log",
        string.format("[PM] Autoload complete: %d loaded, %d failed, %d disabled", nOk, nFail, nSkipped))
end
...
pcall(function()
    if g_tSysConfig then
        syscall("reg_set_value", "@VT\\SYS\\CONFIG", "Hostname", g_tSysConfig.hostname or "AxisBox", "STR")
        if g_tSysConfig.logging then
            for k, v in pairs(g_tSysConfig.logging) do
                syscall("reg_set_value", "@VT\\SYS\\CONFIG", "logging." .. k, tostring(v), "STR")
            end
        end
    end
    syscall("reg_set_value", "@VT\\SYS\\BOOT", "SafeMode", env.SAFE_MODE and "true" or "false", "STR")
    syscall("reg_set_value", "@VT\\SYS\\BOOT", "InitPath", env.INIT_PATH or "/bin/init.lua", "STR")
end)
    ...
    while #g_tPmSignalBuffer > 0 do
        ...
        if sBufSig == "syscall" then
            ...
            if sName == "vfs_open" then
                result1, result2 = vfs_state.handle_open(nCaller, sSynToken, tArgs[1], tArgs[2])
            elseif sName == "vfs_write" then
                result1, result2 = vfs_state.handle_write(nCaller, sSynToken, tArgs[1], tArgs[2])
            elseif sName == "vfs_read" then
                result1, result2 = vfs_state.handle_read(nCaller, sSynToken, tArgs[1], tArgs[2])
            elseif sName == "vfs_close" then
                result1, result2 = vfs_state.handle_close(nCaller, sSynToken, tArgs[1])
            ...
            elseif sName == "vfs_device_control" then
                result1, result2 = vfs_state.handle_device_control(nCaller, sSynToken, tArgs[1], tArgs[2], tArgs[3])
