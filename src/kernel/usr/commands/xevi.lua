...
do
    local ok, mod = pcall(function()
        return require("vi/highlight")
    end)
    if ok and type(mod) == "table" then
        HL = mod
    else
        -- Diagnostic: print why highlighting failed (visible in :messages)
        pcall(function()
            io.write("\27[33m[xevi] Syntax highlight unavailable: " ..
                tostring(mod) .. "\27[37m\n")
        end)
    end
end
...
local TAB_SIZE = 2
local TREE_W = 22
local tTooltip = nil -- nil=hidden, {x,y,tLines,sWord}=visible
...
local function ulen(s)
    return _uc and _uc.len(s) or #s
end
local function usub(s, i, j)
    return _uc and _uc.sub(s, i, j) or s:sub(i, j)
end
...
local function prevCharB(s, p)
    p = p - 1
    while p > 1 and s:byte(p) >= 0x80 and s:byte(p) < 0xC0 do
        p = p - 1
    end
    return math.max(1, p)
end
...
-- =============================================
-- IDENTIFIER BYTE CHECK (for tooltip word extraction)
-- =============================================

local function isIdByte(b)
...
local function lruOldest()
    return buf.lru[1]
end
local function lruRemove(n)
    for i = #buf.lru, 1, -1 do
        if buf.lru[i] == n then
            table.remove(buf.lru, i);
            break
        end
    end
end
...
local function readSwap(nP)
    ...
    sA = nil
    while #t > (buf.meta[nP] or 1) and #t > 1 and t[#t] == "" do
        t[#t] = nil
    end
    if #t == 0 then
        t[1] = ""
...
local function readOrigPage(nP)
    ...
    if not off or not buf.origPath then
        return {""}
    end
...
local function lineToPage(nLine)
    ...
    for i = 1, buf.nPages do
        ...
        if c > 0 and nLine <= acc + c then
            return i, nLine - acc
        end
        acc = acc + c
    end
    if buf.nPages == 0 then
        return 1, 1
    end
...
local function insertLine(nAfter, s)
    ...
    if nAfter < 1 then
        ...
        nP, nI = lineToPage(math.min(nAfter, buf.nTotal))
    end
    local pg = ensurePage(nP)
...
local function openFile(sPath)
    ...
    if not h then
        return false, "Cannot open: " .. sPath
    end
    buf.origPath = sPath;
    buf.sPath = sPath
    buf.meta = {};
    ...
    fs.close(h)
    if not bEndsNL and nByte > 0 then
        nLIP = nLIP + 1;
        buf.nTotal = buf.nTotal + 1
    end
    if nLIP > 0 then
        buf.meta[nPN] = nLIP;
        buf.offsets[nPN] = {nPSB, nByte - nPSB}
...
local function saveFile(sPath)
    ...
    if not h then
        return false, "Cannot write: " .. sPath
    end
    for nP = 1, buf.nPages do
        if (buf.meta[nP] or 0) > 0 then
            local tL
            if buf.cache[nP] then
                tL = buf.cache[nP].lines
            elseif buf.swapped[nP] then
                tL = readSwap(nP)
            else
                tL = readOrigPage(nP)
            end
    ...
    buf.swapped = {}
    for _, pg in pairs(buf.cache) do
        pg.dirty = false
    end
    buf.modified = false;
    buf.sPath = sPath
...
local function undoPush(op, ln, old)
    ...
    if nFree < 65536 then
        mx = 3
    elseif nFree < 131072 then
        mx = 5
    end
    while #tUndo > mx do
        table.remove(tUndo, 1)
    end
end
local function undoPop()
    if #tUndo == 0 then
        sMsg = "Already at oldest change";
        return
    end
    local u = table.remove(tUndo)
    if u.op == "set" then
        setLine(u.ln, u.old)
    elseif u.op == "ins" then
        deleteLine(u.ln)
    elseif u.op == "del" then
        insertLine(u.ln - 1, u.old)
    end
    buf.modified = true
end
...
local function treeRefresh()
    ...
    if not tTreeEntries then
        tTreeEntries = {}
    end
    if nTreeSel > #tTreeEntries then
        nTreeSel = math.max(1, #tTreeEntries)
    end
end

local function treeToggle()
    bTreeOpen = not bTreeOpen
    if bTreeOpen and not tTreeEntries then
        treeRefresh()
    end
end
...
local ctx = xe.createContext({
    ...
    extensions = {"XE_ui_shadow_buffering_render_batch", "XE_ui_diff_render_feature", "XE_ui_alt_screen_query",
                  "XE_ui_deferred_clear", "XE_ui_dirty_row_tracking", "XE_ui_run_length_grouping", "XE_ui_toast"}
})
if not ctx then
    print("xevi: no XE context");
    return
end
...
local function updateSuggestions()
    ...
    table.sort(tCmdSugg, function(a, b)
        return a.score > b.score
    end)
    while #tCmdSugg > nSuggMax do
        tCmdSugg[#tCmdSugg] = nil
    end
    if nSuggSel > #tCmdSugg then
        nSuggSel = #tCmdSugg
    end
end
...
-- =============================================
-- TOOLTIP SYSTEM (K key in normal mode)
-- Scans buffer for function definitions and --- doc comments.
-- No LSP, no external process — pure in-buffer search.
-- =============================================

local function wordAtCursor()
    local s = getLine(nCL)
    if #s == 0 or nCC > #s then
        return nil
    end
    if not isIdByte(s:byte(nCC)) then
        return nil
    end
    local ns = nCC
    while ns > 1 and isIdByte(s:byte(ns - 1)) do
        ns = ns - 1
    end
    local ne = nCC
    while ne < #s and isIdByte(s:byte(ne + 1)) do
        ne = ne + 1
    end
    return s:sub(ns, ne)
end

local function findDefinitions(sWord)
    if not sWord or #sWord == 0 then
        return nil
    end
    local tR = {}
    local nFrom = math.max(1, nCL - 400)
    local nTo = math.min(buf.nTotal, nCL + 400)
    for i = nFrom, nTo do
        local s = getLine(i)
        -- 1) function word(params)
        -- 2) function path.word(params) or path:word(params)
        -- 3) word = function(params)
        -- 4) local word = function(params)
        local sP
        sP = s:match("function%s+" .. sWord .. "%s*%(([^%)]*)%)")
        if not sP then
            sP = s:match("function%s+[%w_%.]+[%.:]" .. sWord .. "%s*%(([^%)]*)%)")
        end
        if not sP then
            sP = s:match(sWord .. "%s*=%s*function%s*%(([^%)]*)%)")
        end
        if not sP then
            sP = s:match("local%s+" .. sWord .. "%s*=%s*function%s*%(([^%)]*)%)")
        end
        if sP then
            -- Collect --- doc comments above the definition
            local tDoc = {}
            for j = i - 1, math.max(nFrom, i - 15), -1 do
                local sD = getLine(j)
                local sDocBody = sD:match("^%s*%-%-%-(.*)")
                if sDocBody then
                    table.insert(tDoc, 1, sDocBody)
                elseif sD:match("^%s*$") then
                    -- blank line: keep scanning (one gap allowed)
                else
                    break
                end
            end
        ...
        -- 5) Table/variable: local word = { or word = {
        if not sP then
            if s:match("local%s+" .. sWord .. "%s*=%s*{") or s:match(sWord .. "%s*=%s*{") then
                tR[#tR + 1] = {
                    line = i,
                    kind = "table"
                }
            elseif s:match("local%s+" .. sWord .. "%s*=") then
                tR[#tR + 1] = {
                    line = i,
                    kind = "local"
                }
...
local function showTooltip()
    local sW = wordAtCursor()
    if not sW then
        tTooltip = nil;
        sMsg = "No word under cursor";
        return
    end
    local tDefs = findDefinitions(sW)
    if not tDefs then
        tTooltip = nil;
        sMsg = "No definition: " .. sW;
        return
    end
    local tL = {}
    local d = tDefs[1]
    if d.kind == "table" then
        tL[1] = sW .. "  (table)"
        tL[2] = "Line " .. d.line
    elseif d.kind == "local" then
        tL[1] = "local " .. sW
        tL[2] = "Line " .. d.line
    elseif d.params then
        tL[1] = sW .. "(" .. d.params .. ")"
        tL[2] = "Line " .. d.line
        if d.docs and #d.docs > 0 then
            -- Has doc comments: show them with formatting
            tL[#tL + 1] = ""
            for _, sDoc in ipairs(d.docs) do
                local sT = sDoc:match("^%s*(.-)%s*$") or sDoc
                if #sT > 0 then
                    tL[#tL + 1] = sT
                end
            end
        else
            -- No doc comments: auto-generate from signature
            local sParams = d.params:match("^%s*(.-)%s*$") or d.params
            if #sParams > 0 then
                tL[#tL + 1] = ""
                for sParam in sParams:gmatch("[^,]+") do
                    local sP = sParam:match("^%s*(.-)%s*$") or sParam
                    if #sP > 0 then
                        tL[#tL + 1] = "@param " .. sP
                    end
                end
    ...
    local nTx = editStartX() + gutW() + nCC - nLeft
    local nTy = (nCL - nTop) + 3
    tTooltip = {
        x = nTx,
        y = nTy,
        tLines = tL,
...
local function ensureVis()
    if nCL < nTop + SCROLL_OFF then
        nTop = math.max(1, nCL - SCROLL_OFF)
    end
    if nCL >= nTop + EDIT_H - SCROLL_OFF then
        nTop = nCL - EDIT_H + SCROLL_OFF + 1
    end
    local tw = editW()
    if nCC < nLeft then
        nLeft = nCC
    end
    if nCC >= nLeft + tw then
        nLeft = nCC - tw + 1
    end
end
...
local function recomputeState()
    ...
    if not HL or not buf.tLang then
        return
    end
    ...
    for i = nFrom, nTo do
        ...
        while p <= #s do
            if bIn then
                ...
                if p + #sBS - 1 <= #s and s:sub(p, p + #sBS - 1) == sBS then
                    bIn = true;
                    p = p + #sBS
                else
                    p = p + 1
...
local function renderTree()
    ...
    if #sTitle > tw - 2 then
        sTitle = sTitle:sub(1, tw - 5) .. "..."
    end
    ctx:text(2, 1, " " .. sTitle .. " ", C.startAcc, C.treeBg)
    for row = 1, EDIT_H do
        ctx:text(tw, row, "|", C.treeBorder, C.treeBg)
    end
    ...
    for i = 1, nVis do
        ...
        local icon = ""
        if e.isDir then
            icon = e.expanded and "v " or "> "
        end
        local sLabel = indent .. icon .. e.name
        if #sLabel > tw - 3 then
            sLabel = sLabel:sub(1, tw - 6) .. "..."
        end
...
local function renderEditor()
    if buf.bStateDirty then
        recomputeState()
    end
    ensureVis()
    if nTop ~= nPrevBufTop then
        nScrollCooldown = 3;
        nPrevBufTop = nTop
    end
    if nScrollCooldown > 0 then
        nScrollCooldown = nScrollCooldown - 1
    end
    ...
    if buf.tSegCache then
        ...
        for nL in pairs(buf.tSegCache) do
            if nL < nLo or nL > nHi then
                buf.tSegCache[nL] = nil
            end
        end
    end
    ...
    local bCanSyntax = (not bScrolling) and HL ~= nil and buf.tLang ~= nil and buf.tLang.name ~= "Text" and
                           computer.freeMemory() > 32768 -- lowered threshold from 49152
    ...
    for row = 1, EDIT_H do
        ...
        if nLine > buf.nTotal then
            ...
            if bCur then
                ctx:fill(esx + gw, row, tw, 1, " ", C.fg, C.curLn)
            end
            local nX = esx + gw
            if bCanSyntax then
                ...
                local sV = ""
                if #sLine >= nLeft then
                    sV = sLine:sub(nLeft, nLeft + tw - 1)
                end
                if #sV < tw then
                    sV = sV .. string.rep(" ", tw - #sV)
                end
...
local function renderStatusBar()
    ...
    if sMode == "start" then
        ctx:text(2, STATUS_Y, " xevi 4.0 ", C.barFg, C.barBg);
        return
    end
    local tM = {
        normal = {" NORMAL ", C.modeN},
        insert = {" INSERT ", C.modeI},
        command = {" COMMAND ", C.modeC},
        search = {" SEARCH ", C.modeS},
...
local function renderSuggestions()
    if sMode ~= "command" or #tCmdSugg == 0 then
        return
    end
...
local function prevWord(s, c)
    c = c - 1;
    while c > 1 and s:sub(c, c) == " " do
        c = c - 1
    end
    while c > 1 and s:sub(c - 1, c - 1) ~= " " do
        c = c - 1
    end
    return c
end
...
local function handleNormal(k)
    ...
    if k == "h" or k == "\27[D" then
        nCC = prevCharB(curL(), nCC);
        fixCol()
    elseif k == "l" or k == "\27[C" then
        nCC = nextCharB(curL(), nCC);
        fixCol()
    elseif k == "j" or k == "\27[B" then
        nCL = math.min(nCL + 1, buf.nTotal);
        fixCol()
    elseif k == "k" or k == "\27[A" then
        nCL = math.max(nCL - 1, 1);
        fixCol()
    elseif k == "0" or k == "\27[H" then
        nCC = 1
    elseif k == "$" or k == "\27[F" then
        nCC = #curL();
        fixCol()
    elseif k == "w" then
        nCC = nextWord(curL(), nCC);
        fixCol()
    elseif k == "b" then
        nCC = prevWord(curL(), nCC);
        fixCol()
    elseif k == "G" then
        nCL = buf.nTotal;
        fixCol()
    elseif k == "g" then
        sPend = "g"
    elseif k == "\27[5~" then
        nCL = math.max(1, nCL - EDIT_H);
        fixCol()
    elseif k == "\27[6~" then
        nCL = math.min(buf.nTotal, nCL + EDIT_H);
        fixCol()
    elseif k == "i" then
    ...
    elseif k == ":" then
        ...
        updateSuggestions()
    elseif k == "\7" then
        sMsg = string.format('"%s" %s%dL %dC', buf.sPath or "[No Name]", buf.modified and "[+] " or "", buf.nTotal,
            #curL())
    end
end
...
local function handleInsert(k)
    ...
    if k == "\b" then
        if nCC > 1 then
            ...
            nCC = pv
        elseif nCL > 1 then
            local sPrev = getLine(nCL - 1);
            nCC = #sPrev + 1
            setLine(nCL - 1, sPrev .. s);
            deleteLine(nCL);
            nCL = nCL - 1
        end
    elseif k == "\n" then
    ...
    elseif k == "\t" then
        ...
        setLine(nCL, s:sub(1, nCC - 1) .. sTab .. s:sub(nCC));
        nCC = nCC + #sTab
    elseif k == "\27[A" then
        nCL = math.max(1, nCL - 1);
        fixCol()
    elseif k == "\27[B" then
        nCL = math.min(buf.nTotal, nCL + 1);
        fixCol()
    elseif k == "\27[D" then
        nCC = prevCharB(s, nCC);
        if nCC < 1 then
    ...
    elseif k and #k >= 1 and k:byte() >= 32 then
        setLine(nCL, s:sub(1, nCC - 1) .. k .. s:sub(nCC));
        nCC = nCC + #k
    end
end
...
local function execCmd(sCmd)
    ...
    if sC == "w" or sC == "write" then
        ...
        if ok then
            ctx:toastSuccess("Written: " .. (buf.sPath:match("[^/]+$") or buf.sPath))
        else
            ctx:toastError(err or "Write failed")
        end
    elseif sC == "q" or sC == "quit" then
        if buf.modified then
            sMsg = "Unsaved changes (use :q!)"
        else
            bRun = false
        end
    elseif sC == "q!" then
    ...
    elseif sC == "set" then
        if sA == "number" or sA == "nu" then
            cfg.lineNumbers = true
        elseif sA == "nonumber" or sA == "nonu" then
            cfg.lineNumbers = false
        elseif sA and sA:match("^tabsize=") then
            TAB_SIZE = tonumber(sA:match("=(%d+)")) or 2
        else
            sMsg = "set: " .. tostring(sA)
        end
    elseif sC == "qa" or sC == "qall" then
        bRun = false
    elseif tonumber(sC) then
        nCL = clamp(tonumber(sC), 1, buf.nTotal);
        fixCol()
    else
        sMsg = "Unknown: " .. sCmd
    end
...
local function handleTree(k)
    ...
    if k == "\27" or k == "q" then
        sMode = (buf.nTotal > 0 and buf.sPath) and "normal" or "start";
        return
    end
...
if sArg then
    ...
    if tL and type(tL) == "table" then
        ...
        if ok then
            ...
            ctx:toastInfo((buf.sPath:match("[^/]+$") or sArg) .. " — " .. buf.nTotal .. "L, " .. buf.nPages ..
                              " pages")
        else
            ctx:toastError(err or "Open failed")
            initEmpty();
