...
local function closeBuffer(nIdx)
    ...
    -- Delete swap file
    if buf._swapPath then fs.remove(buf._swapPath) end
    ...
    if #tBuffers == 0 then
        nActive = 0
    elseif nActive > #tBuffers then
        nActive = #tBuffers
    end

    -- Swap in new active if needed
    if nActive > 0 and tBuffers[nActive] and tBuffers[nActive]._swapped then
        swapIn(tBuffers[nActive], nActive)
    end
...
local function switchBuffer(n)
    if n < 1 or n > #tBuffers or n == nActive then return end

    -- Swap out current buffer to disk
    if nActive >= 1 and nActive <= #tBuffers then
        swapOut(tBuffers[nActive], nActive)
    end
    ...
    -- Swap in new buffer from disk
    if tBuffers[nActive]._swapped then
        swapIn(tBuffers[nActive], nActive)
    end
...
local function undoPush(buf, tItems)
    -- Dynamically cap undo depth based on available memory
    local nMax = cfg.maxUndo
    local nFree = computer.freeMemory()
    if nFree < 65536 then nMax = 3
    elseif nFree < 131072 then nMax = 8 end

    buf.tUndo[#buf.tUndo + 1] = {items = tItems}

    -- Evict oldest entries
    while #buf.tUndo > nMax do
...
-- =============================================
-- 7b. DISK-BACKED BUFFER SWAP
-- Only the ACTIVE buffer lives in RAM.
-- Inactive buffers are serialized to /tmp/.xevi/
-- =============================================
...
local function swapOut(buf, nIdx)
    if not buf.tLines or buf._swapped then return end

    -- Write lines to disk as single string
    local sSwap = SWAP_DIR .. "/buf_" .. nIdx
    local h = fs.open(sSwap, "w")
    if h then
        -- Write in chunks to avoid building one huge string
        for i = 1, #buf.tLines do
            fs.write(h, buf.tLines[i])
            if i < #buf.tLines then fs.write(h, "\n") end
        end
        fs.close(h)
    end

    -- Save cursor/scroll state (tiny — stays in RAM)
    buf._savedCL   = buf.nCL
    buf._savedCC   = buf.nCC
    buf._savedTop  = buf.nTop
    buf._savedLeft = buf.nLeft
...
local function swapIn(buf, nIdx)
    if not buf._swapped then return end

    local sSwap = buf._swapPath or (SWAP_DIR .. "/buf_" .. nIdx)
    local tLines = {""}

    local h = fs.open(sSwap, "r")
    if h then
        local tC = {}
        while true do
        ...
        if #sAll > 0 then
            ...
            sAll = nil  -- free immediately
            if #tLines > 1 and tLines[#tLines] == "" then tLines[#tLines] = nil end
        end
    end
    ...
    -- Restore cursor
    buf.nCL   = math.max(1, math.min(buf._savedCL or 1, #tLines))
    buf.nCC   = math.max(1, buf._savedCC or 1)
    buf.nTop  = buf._savedTop or 1
    buf.nLeft = buf._savedLeft or 1

    buf._swapped    = false
    buf.bStateDirty = true
end

local function swapCleanup()
    -- Delete all swap files on exit
    local tList = fs.list(SWAP_DIR)
    if tList then
        for _, sName in ipairs(tList) do
            local sClean = sName:gsub("/$", "")
            fs.remove(SWAP_DIR .. "/" .. sClean)
        end
    end
    fs.remove(SWAP_DIR)
end
...
local function loadFile(sPath)
    ...
    fs.close(h)
    local sAll = table.concat(tC)
    tC = nil  -- free chunk table immediately

    -- Memory safety: refuse files that would consume >40% of free RAM
    local nFree = computer.freeMemory()
    if #sAll * 2.5 > nFree then  -- 2.5x accounts for line-split overhead
        local nMaxKB = math.floor(nFree * 0.4 / 1024)
        ctx:toastError("File too large (" .. math.floor(#sAll/1024) ..
                       "KB) — max ~" .. nMaxKB .. "KB")
        sAll = nil
        collectgarbage("collect")
        return nil
    ...
    sAll = nil  -- free raw string immediately
...
local function releaseTemps()
    -- Break references to temporary segment tables from last frame.
    -- OC's automatic GC runs between pullSignal() calls;
    -- nilling these ensures the dead tables are collectible.
    local buf = B()
    if not buf then return end
    
    -- The tSegCache entries for lines we JUST rendered hold references
    -- to the segment arrays from HL.segments(). The cache retains them
    -- intentionally — but the LOCAL variables in renderEditor that also
    -- pointed to tSegs are already dead (function returned).
    --
    -- The real waste is the HL.segments() return tables for lines that
    -- were evicted from cache mid-frame. Force a nil sweep on the
    -- scratch variables the Lua VM can't see are dead yet.
    
    -- Shrink undo if memory is tight
    if buf.tUndo and #buf.tUndo > 5 then
        local nFree = computer.freeMemory()
        if nFree < 65536 then
            while #buf.tUndo > 3 do
                table.remove(buf.tUndo, 1)
            end
        end
    end
...
local function renderEditor()
    ...
    -- Evict off-screen syntax cache before rendering
    if buf.tSegCache then
        local nLo = buf.nTop - EDIT_H
        local nHi = buf.nTop + EDIT_H * 2
        for nLine in pairs(buf.tSegCache) do
            if nLine < nLo or nLine > nHi then
                buf.tSegCache[nLine] = nil
            end
        end
    end
    ...
    -- Detect scrolling: suppress syntax for a few frames
    if buf.nTop ~= nPrevBufTop then
        nScrollCooldown = 3
        nPrevBufTop = buf.nTop
    end
    ...
    -- PRE-FILL entire edit area with background.
    -- This guarantees every cell has a delta entry — no gaps,
    -- no stale content from previous scroll positions.
    ctx:fill(1, EDIT_Y, nGW, EDIT_H, " ", CLR.gutter, CLR.gutterBg)
    ctx:fill(nGW + 1, EDIT_Y, nTW, EDIT_H, " ", CLR.fg, CLR.bg)

    -- Now render content ON TOP of the pre-fill.
    -- Cells that match the pre-fill produce zero batch entries (diff skips them).
    -- Cells with actual content overwrite the fill in the delta buffer.
    for row = 1, EDIT_H do
        ...
        if nLine <= #buf.tLines then
            ...
            if cfg.lineNumbers then
                ...
                ctx:text(1, nY, string.rep(" ", nGW - 1 - #sN) .. sN .. " ",
                         CLR.gutter, CLR.gutterBg)
            end

            -- Current line background (overwrite pre-fill for cursor line)
            if bCur then
                ctx:fill(nGW + 1, nY, nTW, 1, " ", CLR.fg, CLR.curLine)
            end

            -- Content
            local nX = nGW + 1
            local bUseSyntax = (not bScrolling) and HL and buf.tLang
                               and computer.freeMemory() > 65536
            ...
            if bUseSyntax then
                ...
                local tSegs
                if cached and cached[1] == cKey
                   and cached[3] == buf.nLeft and cached[4] == nTW then
                    tSegs = cached[2]
                else
                ...
                -- Plain text: ONE string, ONE batch entry per line
                local sV
                if #sLine >= buf.nLeft then
                    sV = sLine:sub(buf.nLeft, buf.nLeft + nTW - 1)
                else
                    sV = ""
                end
            ...
            if bCur then
                ...
                if nCX >= nGW + 1 and nCX <= nGW + nTW then
                    local sC = (buf.nCC >= 1 and buf.nCC <= #sLine)
                              and sLine:sub(buf.nCC, buf.nCC) or " "
                    ctx:text(nCX, nY, sC, CLR.bg, CLR.cursor)
                end
            end
        else
            -- Past EOF: tilde (pre-fill already cleared the rest)
...
while bRunning do
    ctx:beginFrame()
    -- Low memory warning (check every ~60 frames to avoid overhead)
    if not _memCheck then _memCheck = 0 end
    _memCheck = _memCheck + 1
    if _memCheck >= 60 then
        _memCheck = 0
        local nFree = computer.freeMemory()
        if nFree < 32768 then
            ctx:toastWarn("LOW MEMORY: " .. math.floor(nFree / 1024) .. "KB free")
        end
    end
