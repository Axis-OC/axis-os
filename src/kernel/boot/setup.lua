...
local function secureBootSetup(loaderCfg)
    ...
    while true do
        ...
        local items = {
            ...
            {label = ">> Import PKI Keys (from sign -g)", value = (hasPrivKey and hasPubKey) and "Present" or "(run sign -g)"},
        ...
        -- Generate Key Pair
        -- Import PKI Keys (read from /etc/signing/, generated by `sign -g`)
        if idx == 7 then
            cls(BLACK, GREEN)
            center(H/2 - 2, "Reading PKI Keys from /etc/signing/...", CYAN)

            local bFoundPub = rootFs and rootFs.exists("/etc/signing/public.key")
            local bFoundPriv = rootFs and rootFs.exists("/etc/signing/private.key")

            if bFoundPub and bFoundPriv then
                -- Read public key to compute fingerprint
                local hPub = rootFs.open("/etc/signing/public.key", "r")
                local sPubB64 = ""
                if hPub then
                    while true do
                        local c = rootFs.read(hPub, 4096)
                ...
                if #sPubB64 > 0 and hasDC then
                    local fp = hex(dataCard.sha256(sPubB64))
                    eepCfg.pk_fingerprint = fp:sub(1, 32)
                    hasPrivKey = true; hasPubKey = true

                    center(H/2, "Keys imported from /etc/signing/", GREEN)
                    center(H/2 + 1, "Fingerprint: " .. fp:sub(1, 32) .. "...", YELLOW)
                else
                    center(H/2, "Key file empty or no data card", RED)
                end
            else
                center(H/2 - 1, "Keys NOT FOUND in /etc/signing/", RED)
                center(H/2 + 1, "Generate them from the OS first:", YELLOW)
                center(H/2 + 2, "sign -g", WHITE)
            end
            center(H/2 + 4, "Press any key...", GRAY)
            pullKey()
        end
