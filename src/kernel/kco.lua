...
kernel.tSyscallTable["exsi_create"] = {
    func = function(nPid, sCode, tOpts)
        if not g_oExsi then return nil, "EXSi not loaded" end
        return g_oExsi.CreateEnclave(nPid, sCode, tOpts)
    end,
    allowed_rings = {0, 1, 2, 2.5, 3}
}
...
kernel.tSyscallTable["exsi_attest"] = {
    func = function(nPid, nHandle, sUserData)
        if not g_oExsi then return nil, "EXSi not loaded" end
        return g_oExsi.Attest(nHandle, sUserData)
    end,
    allowed_rings = {0, 1, 2, 2.5, 3}
}
...
kernel.tSyscallTable["exsi_channel_open"] = {
    func = function(nPid, nHandleA, nHandleB)
        if not g_oExsi then return nil, "EXSi not loaded" end
        return g_oExsi.OpenChannel(nPid, nHandleA, nHandleB)
    end,
    allowed_rings = {0, 1, 2, 2.5, 3}
}

kernel.tSyscallTable["exsi_channel_send"] = {
    func = function(nPid, nChannelId, nSrcHandle, sData)
        if not g_oExsi then return nil, "EXSi not loaded" end
        return g_oExsi.ChannelSend(nPid, nChannelId, nSrcHandle, sData)
    end,
    allowed_rings = {0, 1, 2, 2.5, 3}
}

kernel.tSyscallTable["exsi_channel_close"] = {
    func = function(nPid, nChannelId)
        if not g_oExsi then return nil, "EXSi not loaded" end
        return g_oExsi.CloseChannel(nPid, nChannelId)
    end,
    allowed_rings = {0, 1, 2, 2.5, 3}
}
...
-- =============================================
-- OLR + KSR PARTITION DISCOVERY & BOOT EXTEND
-- =============================================

local g_tOlrIO = nil   -- OLR I/O handle (sector-based or file-based)
local g_tKsrIO = nil   -- KSR I/O handle

do
    -- For AXFS boot: scan RDB for OLR/KSR partitions
    if g_bAxfsRoot and boot_drive_addr then
        kprint("info", "[OLR/KSR] Scanning RDB for secure partitions...")
        pcall(function()
            local oDrv = raw_component.proxy(boot_drive_addr)
            local ss = oDrv.getSectorSize()
            local sRdb = oDrv.readSector(1)  -- RDB at sector 0 (OC 1-indexed)
            if sRdb and sRdb:sub(1, 4) == "RDSK" then
                local B_r32 = function(s, o)
                    return s:byte(o)*16777216 + s:byte(o+1)*65536
                         + s:byte(o+2)*256 + s:byte(o+3)
                end
                local B_ri32 = function(s, o)
                    local u = B_r32(s, o)
                    return u >= 2147483648 and u - 4294967296 or u
                end
                local ns = B_ri32(sRdb, 25)
                for _ = 1, 16 do
                    if ns < 0 then break end
                    local q = oDrv.readSector(ns + 1)
                    if not q or q:sub(1, 4) ~= "PART" then break end
                    local fsT = B_r32(q, 65)
                    local pOff = B_r32(q, 57)
                    local pSz  = B_r32(q, 61)
                    if fsT == 0x41584F4C then  -- AXOL
                        g_tOlrIO = {
                            readSector = function(n) return oDrv.readSector(pOff + n + 1) end,
                            writeSector = function(n, d) return oDrv.writeSector(pOff + n + 1, d) end,
                            sectorSize = ss, _off = 0,
                        }
                        kprint("ok", "[OLR] Found AXOL partition at sector " .. pOff)
                    elseif fsT == 0x41584B53 then  -- AXKS
                        g_tKsrIO = {
                            readSector = function(n) return oDrv.readSector(pOff + n + 1) end,
                            writeSector = function(n, d) return oDrv.writeSector(pOff + n + 1, d) end,
                            sectorSize = ss, _off = 0, _size = pSz,
                        }
                        kprint("ok", "[KSR] Found AXKS partition at sector " .. pOff .. " (" .. pSz .. " sectors)")
                    end
                    ns = B_ri32(q, 17)
                end
            end
    ...
    -- Fallback: file-based OLR/KSR
    if not g_tOlrIO then
        g_tOlrIO = { _fs = g_oPrimitiveFs, _path = "/etc/.olr.dat" }
        kprint("info", "[OLR] Using file-based storage (/etc/.olr.dat)")
    end
    if not g_tKsrIO then
        g_tKsrIO = { _fs = g_oPrimitiveFs, _path = "/etc/.ksr.dat" }
        kprint("info", "[KSR] Using file-based storage (/etc/.ksr.dat)")
    end

    -- KSR: Increment boot counter + extend with boot measurement
    pcall(function()
        local sKsrCode = primitive_load("/lib/ksr.lua")
        if sKsrCode then
            local tKE = {
                string=string, math=math, table=table, bit32=bit32,
                os=os, type=type, tostring=tostring, pcall=pcall,
                raw_computer=raw_computer,
                require=function(m)
                    if m=="bpack" then return kernel.custom_require("bpack", 0) end
                    if m=="sha256" then return g_oSha256Lib end
                end,
            }
            local fK = load(sKsrCode, "@ksr", "t", tKE)
            local oKSR = fK()
            local nBoot = oKSR.IncrementBootCounter(g_tKsrIO)
            kprint("info", "[KSR] Boot counter: " .. tostring(nBoot))

            -- Compute kernel hash for measurement
            local sKH = nil
            if g_oSha256Lib then
                local sKC = primitive_load("/kernel.lua")
                if sKC then sKH = g_oSha256Lib.hex(g_oSha256Lib.digest(sKC)) end
            end
            -- Component fingerprint
            local tCFP = {}
            pcall(function()
                for a, t in raw_component.list() do tCFP[#tCFP+1] = t..":"..a end
            end)
            table.sort(tCFP)
            local sCFP = g_oSha256Lib and g_oSha256Lib.hex(g_oSha256Lib.digest(table.concat(tCFP, "|"))) or ""

            oKSR.ExtendBoot(g_tKsrIO, sKH, sCFP)
            kprint("ok", "[KSR] Boot measurement recorded")
        end
    end)
end
...
kernel.tSyscallTable["olr_get_status"] = {
    func = function(nPid)
        if not g_tOlrIO then return { bLocked = false } end
        local sCode = primitive_load("/lib/olr.lua")
        if not sCode then return { bLocked = false } end
        local tE = { string=string, math=math, table=table, bit32=bit32, os=os,
            type=type, tostring=tostring, pcall=pcall, computer=computer,
            raw_component=raw_component, component=component,
            require=function(m)
                if m=="bpack" then return kernel.custom_require("bpack", 0) end
                if m=="sha256" then return g_oSha256Lib end
            end }
        local f = load(sCode, "@olr", "t", tE)
        local oOLR = f()
        return oOLR.GetStatus(g_tOlrIO)
    end,
    allowed_rings = {0, 1, 2, 2.5, 3},
}

kernel.tSyscallTable["olr_verify"] = {
    func = function(nPid, sUser, sPassHash)
        if not g_tOlrIO then return true end
        local sCode = primitive_load("/lib/olr.lua")
        if not sCode then return true end
        local tE = { string=string, math=math, table=table, bit32=bit32, os=os,
            type=type, tostring=tostring, pcall=pcall, computer=computer,
            raw_component=raw_component, component=component,
            require=function(m)
        ...
        local f = load(sCode, "@olr", "t", tE)
        return f().Verify(g_tOlrIO, sUser, sPassHash)
    end,
    allowed_rings = {0, 1},
}

kernel.tSyscallTable["olr_setup"] = {
    func = function(nPid, sUser, sPassHash)
        if not g_tOlrIO then return false, "No OLR partition" end
        local sCode = primitive_load("/lib/olr.lua")
        if not sCode then return false, "olr.lua missing" end
        local tE = { string=string, math=math, table=table, bit32=bit32, os=os,
            type=type, tostring=tostring, pcall=pcall, computer=computer,
            raw_component=raw_component, component=component,
            require=function(m)
        ...
        local f = load(sCode, "@olr", "t", tE)
        local nBC = 0
        pcall(function() nBC = f().GetStatus(g_tOlrIO).nGen or 0 end)
        return f().Setup(g_tOlrIO, sUser, sPassHash, nBC)
    end,
    allowed_rings = {0, 1},
}

kernel.tSyscallTable["olr_clear"] = {
    func = function(nPid, sUser, sPassHash)
        if not g_tOlrIO then return true end
        local sCode = primitive_load("/lib/olr.lua")
        if not sCode then return true end
        ...
        local f = load(sCode, "@olr", "t", tE)
        return f().Clear(g_tOlrIO, sUser, sPassHash)
    end,
    allowed_rings = {0, 1},
}

kernel.tSyscallTable["olr_force_clear"] = {
    func = function(nPid)
        if not g_tOlrIO then return true end
        local sCode = primitive_load("/lib/olr.lua")
        if not sCode then return true end
        ...
        local f = load(sCode, "@olr", "t", tE)
        return f().ForceClear(g_tOlrIO)
    end,
    allowed_rings = {0},  -- BIOS Setup context only
}
...
kernel.tSyscallTable["ksr_get_boot_counter"] = {
    func = function(nPid)
        local sCode = primitive_load("/lib/ksr.lua")
        if not sCode then return 0 end
        local tE = { string=string, math=math, table=table, bit32=bit32, os=os,
            type=type, tostring=tostring, pcall=pcall, raw_computer=raw_computer,
            require=function(m)
                if m=="bpack" then return kernel.custom_require("bpack", 0) end
                if m=="sha256" then return g_oSha256Lib end
            end }
        local f = load(sCode, "@ksr", "t", tE)
        return f().GetBootCounter(g_tKsrIO)
    end,
    allowed_rings = {0, 1, 2, 2.5, 3},
}

kernel.tSyscallTable["ksr_extend_quarantine"] = {
    func = function(nPid, sDrvName, nFaults)
        local sCode = primitive_load("/lib/ksr.lua")
        if not sCode then return false end
        local tE = { string=string, math=math, table=table, bit32=bit32, os=os,
            type=type, tostring=tostring, pcall=pcall, raw_computer=raw_computer,
            require=function(m)
                if m=="bpack" then return kernel.custom_require("bpack", 0) end
        ...
        local f = load(sCode, "@ksr", "t", tE)
        return f().ExtendQuarantine(g_tKsrIO, sDrvName, nFaults)
    end,
    allowed_rings = {0, 1},
}

kernel.tSyscallTable["ksr_is_quarantined"] = {
    func = function(nPid, sDrvName)
        if not g_tKsrIO or not g_tKsrIO.readSector then return false end
        local sCode = primitive_load("/lib/ksr.lua")
        if not sCode then return false end
        local tE = { string=string, math=math, table=table, bit32=bit32, os=os,
            type=type, tostring=tostring, pcall=pcall, raw_computer=raw_computer,
        ...
        local f = load(sCode, "@ksr", "t", tE)
        return f().IsQuarantined(g_tKsrIO, sDrvName)
    end,
    allowed_rings = {0, 1, 2},
}
