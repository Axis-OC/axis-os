...
-- Falls back to managed filesystem if no drive found or no kernel on AXFS.
...
local oFs
local kcode = nil  -- kernel code, found by whichever path works
...
if pOff then
  p("AXFS partition at sector " .. pOff, 0x55FF55)

  local function prs(n) return drv.readSector(pOff + n + 1) end
  local sb = prs(0)

  if sb and sb:sub(1,4) == "AXFS" then
    local nDS = r16(sb, 20)
    ...
    -- Try reading kernel from AXFS
    p("Reading /kernel.lua from AXFS...", 0xAAAAAA)
    kcode = readfile("/kernel.lua")

    if kcode and #kcode > 100 then
      boot_mode = "axfs"
      p("Kernel: " .. #kcode .. " bytes (AXFS)", 0x55FF55)
    else
      p("No kernel on AXFS, trying managed FS...", 0xFFAA00)
      kcode = nil
    end
  else
    p("Bad AXFS superblock, trying managed FS...", 0xFFAA00)
  end
end

-- =============================================
-- FALLBACK: MANAGED FILESYSTEM
-- =============================================

if not kcode and fs_addr then
  boot_mode = "managed"
  oFs = c.proxy(fs_addr)
  p("Trying managed filesystem...", 0xFFAA00)

  local h = oFs.open("/kernel.lua", "r")
  if h then
    local ch = {}
    while true do
      local s = oFs.read(h, 8192)
      if not s then break end
      ch[#ch + 1] = s
    end
    oFs.close(h)
    kcode = table.concat(ch)
  end

  if kcode and #kcode > 100 then
    p("Kernel: " .. #kcode .. " bytes (Managed)", 0x55FF55)
  else
    kcode = nil
  end
end
...
if not kcode then
  halt("/kernel.lua not found on any device")
end

-- =============================================
-- SET UP KERNEL GLOBALS
-- =============================================

-- These are REQUIRED by kernel.lua
_G.raw_component = c        -- <== WAS MISSING
_G.raw_computer  = cp       -- <== WAS MISSING
_G.component     = c        -- kernel sandbox rebuilds these
_G.computer      = cp
...
_G.boot_args     = {}
_G.boot_security = nil

-- =============================================
-- LAUNCH
-- =============================================

p("Executing kernel...", 0x00BCD4)
cp.pullSignal(0.1)
